...
public abstract class Entity {
    ...
    public boolean isCollidingWith(Entity other) {
        // implement me!
        // throw new IllegalStateException("Hey 102 Student! You need to implement
        // isCollidingWith in Entity!");

        if ((this.x <= other.getX() && this.x + this.width >= other.getX())
                || (this.x >= other.getX() && this.x <= other.getX() + other.getWidth())) { // if given x cord
            return (this.y <= other.getY() && this.y + this.height >= other.getY())
                    || (this.y >= other.getY() && this.y <= other.getY() + other.getHeight());
        }
    ...
    public boolean isCollidingWith(int x, int y) {
        if (this.x <= x && this.x + this.width >= x) { // if given x cord
